...
class Data extends AbstractHelper
{
    ...
    public function getStores($query = null)
    {
        ...
        foreach ($website->getGroups() as $group) {
            ...
            foreach ($stores as $store) {
                $storesResult[$store->getCode()] = array(
                    'name' => $store->getName(),
    ...
    {
        return array('isbn' => 'ISBN', 'school' => 'School', 'title' => 'Title');
    }
    ...
    {
        $theUrlYouSeek = $this->centralHelper->getSchoolSearchUrl(array(), $this->centralHelper->getSslStatus());
    ...
    public function getPathDetails($path)
    {
        ...
        if (count($splitPath) > 1) {
            ...
            $pathObject['the_rest'] = trim(str_replace(array($splitPath[0] . "/", $splitPath[0], "?" . $pathObject['query'], "#" . $pathObject['hash']), $path, ""), "/");
        } else {
    ...
    public function getSchoolSpecificProductData($product, $school = null, $packageGroupId = null)
    {
        ...
        if ($packageGroupId && $productId && $schoolId) {
            ...
            $packages->getSelect()
                ...
                )->where($prefix.'.product_id = ?', $productId
                )->where('g.id = ?', $packageGroupId
                )->where('g.school_id = ?', $schoolId //THIS IS REDUNDANT. [$packageGroupId AND $productId] form a unique recordset
                );
